...
On the OSAP team for the full co-op term, manually testing both the student-facing and administrator-facing sides of the OSAP application across Full-Time, Part-Time, Microcredential, and OLSG program types. Log and track defects through ALM, working a build, test, defect, retest loop directly with developers. Built a set of Python Playwright scripts on my own initiative to speed up the repetitive parts of manual testing, like student profile creation, separate from the team's existing automation tooling.
