...
/*Custom icon: Playwright isn't available yet in the installed version of
  react-icons, so this mirrors the same shape (IconBaseProps in, sized via
  1em, coloured via currentColor) so it drops into the techStack registry
  exactly like every other Si* icon.*/
export default function SiPlaywright(props: IconBaseProps) {
  const { size = "1em", color = "currentColor", title, ...rest } = props;
